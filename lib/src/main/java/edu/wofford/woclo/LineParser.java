...
/**
 * This program parses command line arguemnts. A required argument must appear
 * in the command line
 * in the position it was given. For example, if the user has created a new
 * instance of lineparser
 * called parse: If the user passes in parser.addRequiredArgument(height,
 * Datatype.INTEGER);
 * parser.addRequiredArgument(weight, Datatype.INTEGER); After the using calls
 * parser.parse(commandline), a command line with the values ["1","2"] will set
 * the value of height
 * to 1 and width to 2. These values can be retreived through the method
 * getArgument.
 * parser.getArgument("height") will return 1. A command line with the values
 * ["1"] will return an
 * error. An optional value is a value that may or may not appear on the command
 * line. Optional
 * arguments can be added using addOptionalArgument() method and can be used as
 * flags. The flag -h
...
public class LineParser {
  ...
  /**
   * Constructs an empty map of named arguments. Usage information describes
   * program information,
   * such as prupose and expected values. Usage information will appear at the top
   * of a help
   * message. For example: usage: java EquivalentStrings [-h] string1 string2
   *
   * <p>
   * positional arguments: string1 (string) the first string string2 (string) the
   * second string
   *
   * <p>
  ...
  /**
   * Constructs an empty map of named arguments. Program information will appear
   * in help message,
   * describing the purpose of the program. usage: java EquivalentStrings [-h]
   * string1 string2
   *
   * <p>
  ...
  /**
   * Constructs an argument map based on an xml file, tag represent argument type,
   * name, and
  ...
  /**
   * This program constructs an xml file of the given filename using the map in
   * this instance of
   * LineParser. The xml file will be constructed in a similar format to the xml
   * file given above.
   *
  ...
  /**
   * A private method for getting an argument object with the given key. Cannot be
   * called before
   ...
   * the argument map,
   * declaring a type. A required argument must appear in the command line in the
   * position it was
   * given. For example, if the user has created a new instance of lineparser
   * called parse: If the
   ...
   * parser.parse(commandline), A command line with the values ["1","2"] will set
   * the value of
   * height to 1 and width to 2. These values can be retreived through the method
   * getArgument.
   * parser.getArgument("height") will return 1. A command line with the values
   * ["1"] will return an
   * error.
   *
   * @param name The name of the argument (what its called, a moniker for an entry
   *             in the command
   *             line).
   ...
   * Specify an argument to come through the command line. Adds the argument to
   * the argument map,
   * declaring a type. /** Specify an argument to come through the command line.
   * Adds the argument
   * to the argument map, declaring a type. A required argument must appear in the
   * command line in
   * the position it was given. For example, if the user has created a new
   * instance of lineparser
   * called parse: If the user passes in parser.addRequiredArgument(height,
   * Datatype.INTEGER);
   * parser.addRequiredArgument(wight, Datatype.INTEGER); After the using calls
   ...
   *
   * <p>
   * This constructor adds additional help information to the argument, so that
   * the argument is
   ...
   *
   * <p>
   * This constructor allows additional discreete values to be included. If a
   * value is not a
   * member of discreete values, an error is thrown. A specification of
   * parser.addRequiredArgument("height", Datatype.INTEGER, "", new
   * String[]{"1","2"}) will throw an
   ...
   * values will be set to default. Optional arguments may or may not appear on
   * the command line, an
   * error will not be thrown. There are a few types of optional arguments: long
   * form "-type",
   * shortform "-t", and combined shortform "-tsm". Combined short forms act as
   * flag, they are set
   * as booleans and if they appear in the command line are set to value "true".
   * Boolean do not take
   * a secondary command line value. Their appearance on the command line sets
   * them to true.
   * Otherwise the next positional argument on the command line is taken as a
   * value.
   *
   * <p>
   * To add an argument with this contructor, an example would be:
   ...
   * will set a default value on the optional argument in case it is not passed
   * in.
   *
   * <p>
   * To add an argument with this contructor, an example would be:
   ...
   * will set a default value on the optional argument in case it is not passed
   * in. A help value
   * will specify help information for an optional value so it appears on the
   * help/usage message.
   *
   * <p>
   * To add an argument with this contructor, an example would be:
   * parser.addOptionalArgument("shape", Datatype.String, "rectangle", "The
   ...
   * <p>
   * To add an argument with this contructor, an example would be:
   * parser.addOptionalArgument("triangle", Datatype.BOOLEAN, "false", "The
   * desired shape of the
   * polygon.", "-t");
   *
   * @param name         The name of the argument (what its called).
   ...
   *                   =======
   * @param <T>        Generic type.
   *                   >>>>>>> 1eaf96f26e558cef7ae847e7dc74886ee1697e9c
  ...
  /**
   * Checks if given value has a discrete list, if it does, verify that value is
   * within discreet
   * list.
   *
  ...
  /**
   * Checks discrete values are of proper type. If they are not of the correct
   * type for the
   ...
   * following cases: -If there are more non-named arguments than identifiers. -If
   * there are less
   * non named arguments than identifiers. -If a given argument value cannot be
   * converted to its
   * specified type. -If a named argument is declared but not followed by a value.
   * Shortform
   * collections are exempt from this. After parse has been completed, getArgument
   * can be called on
   * any values that appeared in the command line. If -h or --help appears in the
   * command line,
